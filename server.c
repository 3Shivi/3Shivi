...
#define PORT 11234   //users will be connecting to port no.
...
//now from here function defined
void sigstop_sigkill_handler(int sig) {
	char str[5];
	//signal for stopping the server
...
int main(){
	...
	sa.sin_addr.s_addr = htonl(INADDR_ANY);	//automatically filled with my ip address
	sa.sin_port = htons(PORT);			//long(datatype),conversion from host byteorder to network byte order

	if(bind(sockfd, (struct sockaddr *)&sa, sizeof(sa))==-1){
		printf("Binding Port Failed!!!\n");
		exit(0);
	}
	if(listen(sockfd, 100)==-1){                        //pending request used 100
		printf("Listen Failed!!!\n");
		exit(0);
	}
	while(1){ 
		int connectedfd;
		if((connectedfd = accept(sockfd, (struct sockaddr *)NULL, NULL))==-1){   //accepted the client request
			printf("Connection Error!!!\n");
			exit(0);
		}
		if(fork()==0)                 //for child process
...
int login(int sock){
	...
	char password[PASS_LENGTH];               //defined as 20
	struct account temp;
...
int signup(int sock){                 //signup functionality
...
int menu2(int sock, int id){
	...
	if(op_id == 1){                           //adding train
		...
		temp.av_seats = 20;               //available seats default set as 20
	...
	if(op_id == 2){        //deleting train fuctionality
		int fd = open(TRAIN, O_RDWR);
...
int menu1(int sock, int id, int type){
	...
	if(op_id == 1){
		...
		memset(&temp,0,sizeof(struct train));               //setting temp value as 0
		int trainid, seats;
...
void view_booking(int sock, int id, int type){
	...
	if(fp == 0)
		...
		for(fp=0;fp<entries;fp++){
			int x=bk[fp].seat_end-bk[fp].seat_start+1;
